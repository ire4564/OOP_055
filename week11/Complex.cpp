...
Number *Complex::add(Number * num) {
	...
	if(num->types() == INTEGER){
        ...
        set_val(number->val() + this->real(), this->imag());
		delete num;
		return this;		
	}
        ...
        set_val(number->real() + this->real(), number->imag() +this->imag());
        delete num;
        return this;
	}
	else{
...
Number *Complex::sub(Number * num) {
     ...
     */
     if(num->types() == INTEGER){
		...
		Complex* number = dynamic_cast<Complex*>(num);  
        set_val(this->real() - number->real(), this->imag() - number->imag());
		delete num;
		return this;	
	}
	else{
...
Number *Complex::mul(Number * num) {
     ...
     */
      ...
      if(num->types() == INTEGER){
		Integer* number = dynamic_cast<Integer*>(num);
        set_val(number->val() * this->real(), this->imag() * number->val());
		delete num;
		return this;
        ...
        Complex* number = dynamic_cast<Complex*>(num);  
        set_val(this->real() * number->real() - this->imag() * number->imag(), this->real() * number->imag() + this->imag() * number->real());
		delete num;
		return this;
...
double Complex::imag() const {
     ...
     * 이를 이용해 real 값은 0번째 imag값은 1번째에 있으므로 real, imag값을 얻을 수 있음
     */
    return get<1>(value); // real() 을 참고하여 구현할 것!
}
